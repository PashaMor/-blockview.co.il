...
async function overpass(query, opts) {
  ...
  // How many elements a real answer should have. A rate-limited mirror often
  // replies 200 with an EMPTY list — indistinguishable from "genuinely nothing
  // nearby" and the reason city-centre buildings were saved blank. So we treat
  // an empty answer as suspect and try the other mirrors before believing it.
  const minElements = opts.minElements != null ? opts.minElements : 1;
  let lastErr, lastEmpty = null;
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    try {
      ...
      if (n < minElements && attempt < attempts - 1) { lastEmpty = json; await sleep(1500); continue; }
      return json;
    } catch (e) {
      lastErr = e;
  ...
  if (lastEmpty) return lastEmpty;   // every mirror agreed it's empty — believe it
  throw lastErr;
}
